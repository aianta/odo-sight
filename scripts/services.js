...
var services = (function(){
    ...
    _public.getLogUIAppList = function (){
        return stateManager.host().then(logUIHost=>{
            return axios.get(`${_LOG_UI_PROTOCOL}://${logUIHost}${_LOG_UI_APP_LIST_PATH}`)
            .then(function(response){
                return Promise.resolve(response.data)
            })
            .catch(function(err){
    ...
    _public.getFlightList = function(appId){
        return stateManager.host().then(logUIHost=>{
            return axios.get(`${_LOG_UI_PROTOCOL}://${logUIHost}${_LOG_UI_FLIGHT_LIST_PATH}${appId}/`)
            .then(function(response){
                return Promise.resolve(response.data)
            }).catch(function(err){
    ...
    _public.createFlight = function (appId, flightName, flightDomain){
        return stateManager.host().then(logUIHost=>{
            return axios.post(`${_LOG_UI_PROTOCOL}://${logUIHost}${_LOG_UI_FLIGHT_CREATE(appId)}`,{
    ...
    _public.getJWT = function(username, password){
        return new Promise((resolve, reject)=>{
            ...
            stateManager.host().then(logUIHost=>{
                console.log(`${_LOG_UI_PROTOCOL}://${logUIHost}${_LOG_UI_JWT_PATH}`)
        
                axios.post(`${_LOG_UI_PROTOCOL}://${logUIHost}${_LOG_UI_JWT_PATH}`, {
                    username: username,
                    password: password
                }).then(function(response){
...
axios.interceptors.request.use(function(request){
    ...
    stateManager.host().then(logUIHost=>{
        ...
        if(request.url.includes(logUIHost) &&
            request.url !== `${_LOG_UI_PROTOCOL}://${logUIHost}${_LOG_UI_JWT_PATH}`){
